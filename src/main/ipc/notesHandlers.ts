        ...
        async (_event, html: string, mdFilePath: string, vaultPath: string): Promise<string> => {
            ...
            try {
                ...
                try {
                    ...
                    // Write to a temporary file first, then swap — this avoids
                    // any contention with OS-level locks on the target PDF.
                    ...
                    if (fs.existsSync(pdfPath)) {
                        try {
                            ...
                            // trashItem can fail on some systems — fall back to unlink
                            try { fs.unlinkSync(pdfPath); } catch { /* ignore */ }
                        }
                        // Wait a tick for the OS to fully release the path
